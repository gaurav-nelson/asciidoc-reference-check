...
//get the package settings whenever they change
var cfgCheckURL;
atom.config.observe('asciidoc-reference-check.checkURL', (checkURL) => {
  cfgCheckURL = checkURL;
});
var cfgHttpStatusCodes;
atom.config.observe('asciidoc-reference-check.httpStatusCodes', (httpStatusCodes) => {
  cfgHttpStatusCodes = httpStatusCodes.split(',');
  for (let a in cfgHttpStatusCodes) { // convert to array of integer
...
function checkReferences(filePath, basePath) {
  ...
  reader.on('line', function (line, index, start, end) {
    ...
    if (line.match(/(?:^|<|[\s>\(\)\[\];])((https?|file|ftp|irc):\/\/[^\s\[\]<]*[^\s.,\[\]<\)])/g)) {
      //console.log('LINE-----',line)
      let extractLink = line.match(/((https?|file|ftp|irc):\/\/[^\s\[\]<]*[^\s.,\[\]<\)])/g); //there may be more than one matching items
      for (let i = 0; i < extractLink.length; i++) {
        let newReference = extractLink[i];
  ...
  reader.on('end', () => {
          ...
          process.nextTick(() => {
            ...
            linkCheck(item, opts ,function (err, result) {
              if (result.status === 'alive') {
                ...
                atom.notifications.addError(
                  '`' + logMsgFilePath +
                  '`: Cannot reach URL (status code `' + result.statusCode + '`): `' + item + '`',
                  {
                    dismissable: true,
                  });
                resolve(false);
    ...
    if (cfgCheckURL) {
      if (externalURLs[0] == null) {
        atom.notifications.addSuccess('`' + logMsgFilePath + '`: No URLs found.');
      } else {
        //get uniques so that we only check them once
        externalURLs = uniq(externalURLs);
        Promise.all(externalURLs.map(checkExternalURL)).then(returnCodes => {
          if (!returnCodes.includes(false)) {
            atom.notifications.addSuccess('`' + logMsgFilePath + '`: All URLs are reachable.'
            );
          }
        });
      }
